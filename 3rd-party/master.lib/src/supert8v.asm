...
;
; Description:
;	パターンの表示(8xn限定, 4色以内, 縦反転)
;
; Functions/Procedures:
;	void super_put_tiny_small_vrev( int x, int y, int num ) ;
;
; Parameters:
;	x,y	描画する座標
;	num	パターン番号
...
;	95/ 1/ 5 Initial: supert8v.asm / master.lib 0.23
;
...
func SUPER_PUT_TINY_SMALL_VREV	; super_put_tiny_small_vrev() {
	push	BP
	mov	BP,SP
	push	DS
	push	SI
	...
	mov	BX,BP			; BX=PATTERN_HEIGHT
	mov	BH,0			;
	add	DI,BX
	dec	DI			; DI = (y + PATTERN_HEIGHT - 1)
