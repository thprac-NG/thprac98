...
const char_chunk_t char_chunks[THPRAC_CHAR_CHUNKS_SIZE] = {
    ...
    // For some reason DOSBox-X treats 0x1F as printable, prints a downward
    // arrow.
    ...
    // To correctly print these character, one need to shift from the "kanji
    // mode" (default) to "graph mode", by printing "\x1B)3". Shifting back to
    // kanji mode can be realized by printing "\x1B)0". See
    // https://github.com/nmlgc/dosbox-x/blob/2d40b705937b7536717f8322147b921c0ed6bd25/pc98-testme-1/contents/CHARMODE.C#L28
    // for more details.
    ...
    // 0x8140(òaéöä‘äu, wajikankaku) ~ 0x81AC(â∫ë ãLçÜ, kedakigou). 0x817F
    // skipped. See
    ...
    // (half-width ÅB). This character is ãÂì_(kuten), a Japanese punctuation
    // mark, so it should belong to the next field.
