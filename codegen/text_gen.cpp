...
#error \
    "This file requires a modern C++ with C++11 support to compile." \
...
void code_generate(const std::string& encoded_str) {
  ...
  for (auto&& kv : glossary_json.items()) {
    ...
    if (!value.is_array() || value.size() != 3u || !value[0].is_string() ||
      ...
      throw std::invalid_argument(
          "Glossary '" + key +
          "' in texts.json doesn't map to an array of 3 "
          "strings.");
    }
    glossary_key_set.insert(key);
    glossaries.push_back(glossary_t{
        key, std::array<std::string, 3>({value[0].get<std::string>(),
  ...
  for (auto&& kv : group_json.items()) {
    ...
    auto&& value = kv.value();
    if (!value.is_array()) {
      throw std::invalid_argument("Group '" + key +
                                  "' in texts.json doesn't map to an array.");
    }
    ...
    for (std::size_t i = 0; i < value.size(); ++i) {
      ...
      if (glossary_key_set.find(current.get<std::string>()) ==
        ...
        throw std::invalid_argument("'" + current.get<std::string>() +
  ...
  std::string autogenerated_warning = THPRAC98_TEXT_GEN_WARNING_TEXT;
  textsdef_hpp += autogenerated_warning;

  textsdef_hpp += "#ifndef THPRAC98_SRC_TEXTSDEF_HPP_" ENDL
  ...
  for (std::size_t language = 0; language < 3; ++language) {
    ...
    for (std::size_t i = 0; i < glossaries.size(); ++i) {
      textsdef_cpp +=
          INDENT INDENT "\"" + escape(glossaries[i].texts[language]) + "\"";
      if (i != glossaries.size() - 1) {
        textsdef_cpp += ",";
      }
      textsdef_cpp += ENDL;
